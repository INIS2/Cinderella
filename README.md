...
12시가 땡하면 마법으로 만든 드레스가 사라지고 집에 가야 하는 신데렐라처럼, Windows PC(대여 나갔던 노트북이나 조치가 필요한 PC 등)를 한순간에 순정 상태로 되돌려 놓는 프로그램입니다.

초기화는 다른 사용자의 조작이 묻었던 프로그램을 신뢰하지 않고, 이전 사용자의 조작이나 파일이 다음 사용자와 교차되거나 영향을 주지 않게 하기 위한 작업입니다.

초기화에는 여러 방법이 있지만 각각 장단점이 있습니다.

- 포맷: 확실한 초기화입니다. 원하는 세팅이 틀어지고, 프로그램을 다시 설치하거나 인증받아야 합니다.
- 이미지: 확실한 초기화입니다. 원하는 세팅을 유지할 수 있지만 시간이 오래 걸리고, 업데이트에 맞춰 새로 만들어야 합니다.
- 수동: 불확실한 초기화입니다. 간단하고 빠르게 끝나지만 직접 해야 해서 손이 많이 가고, 놓치는 부분이 있을 수 있습니다.

여기에서 프로젝트 신데렐라가 나옵니다.

일반 사용자가 일시적으로 사용하고 반납한다면 주로 사용하게 되는 파트를 메인으로 조치합니다. 추후에는 변경 감지 및 조치까지 확장합니다.

- 브라우저(주로 Chrome, Edge) 인터넷 사용 기록, 로그, 캐시, 자동 저장, 북마크 등
- 주요 경로(다운로드, 문서, D 드라이브 등)의 파일 제거 및 휴지통 비우기
- 설치 프로그램 대조(기존 목록을 잡아놓고 점검 시점과 비교)
- 환경 설정 대조(레지스트리나 명령어를 통해 화면 시간 등의 기존 설정과 달라진 점 파악)
- 배경화면 구성 및 바로가기 설정

급하게 노트북이 들어왔다 나가야 하는 경우, 여러 대의 노트북을 관리하는 경우 현실적으로 모든 PC를 매번 포맷하기는 어렵습니다. 이때 신데렐라를 통해 일반 사용자가 조작 가능한 수준의 표면적 조치를 우선 수행하고, 분기 1회 등 업데이트가 반영된 새로운 이미지를 만들어 단계별로 적용하면 더 완성도 높은 운영이 가능합니다.
